import os, shlex, subprocess, shutil, platform, signal
...
def main():
    while True:
        ...
        if "|" in input_args:
            ...
            global piped_cmd, curr_cmd, num_cmds

            for token in input_args:
                if token == "|":
                    piped_cmd.append(curr_cmd)
                    curr_cmd = []
                else:
                    curr_cmd.append(token)

            if curr_cmd:
                piped_cmd.append(curr_cmd)

            num_cmds = len(piped_cmd)
        ...
        match command:
            ...
            case "ls":
                try:
                    if hasattr(os, 'fork'):
                        forkAndPipe(num_cmds, piped_cmd)
                ...
                try:
                    if hasattr(os, 'fork'): 
                        ...
                        if pid == 0:
                            print()
                            if len(argument) == 0: 
                                os.execvp('mkdir', ['mkdir'])
                            else:
                                os.execvp('mkdir', ['mkdir', *argument])
                ...
                try:
                    if hasattr(os, 'fork'):
                        ...
                        if pid == 0:
                            print()
                            if len(argument) == 0: 
                                os.execvp('rmdir', ['rmdir'])
                            else:
                                os.execvp('rmdir', ['rmdir', *argument])
                ...
                if hasattr(os, 'fork'):
                    ...
                    if pid == 0:
                        ...
                        try:
                            ...
                            print(f"\n\33[1m\33[91mcat: {e}\33[0m")
                else:
                    if len(argument) == 0:
                        print("\n\33[1m\33[91mcat: missing operand\33[0m")
                        print("\n\33[1m\33[91mTry 'cat --help' for more information\33[0m")
                        continue
                    ...
                    for file in argument:
                        try:
                            ...
                            print(f"\n\33[1m\33[91mcat: {file}: No such file or directory\33[0m")
                        except PermissionError:
                            print(f"\n\33[1m\33[91mcat: {file}: Permission denied\33[0m")
                        except Exception as e:
                            print(f"\n\33[1m\33[91mcat: {e}\33[0m")
            ...
            case "touch":
                if hasattr(os, 'fork'):
                    ...
                    if pid == 0:
                        if len(argument) == 0:
                            os.execvp('touch', ['touch'])
                        else:
                            os.execvp('touch', ['touch', *argument])
                    elif pid > 0:
                        try:
                            finishedpid, status = os.waitpid(pid, 0)
                        except KeyboardInterrupt:
                            ...
                            print(f"\n\33[1m\33[91mtouch: {e}\33[0m")
                else:
                    if len(argument) == 0:
                        print("\n\33[1m\33[91mtouch: missing operand\33[0m")
                        print("\n\33[1m\33[91mTry 'touch --help' for more information\33[0m")
                        continue
            ...
            case ":?":
                ...
                print("  touch                 Create an empty file or update the timestamp of an existing file")

            case _:
                print('\n\33[1m\33[91mbe patient kitten daddy is implementing it next week ;)\33[0m')

def forkAndPipe(num_cmd, pipe_cmd):

    pipes = [os.pipe() for _ in range(num_cmd - 1)]
    pids = []

    for i, parts in enumerate(pipe_cmd):

        if not parts: 
            print(f"Error: Empty command detected at position {i}")
            continue
        ...
        if pid == 0:
            print()
            if i > 0:
                os.dup2(pipes[i - 1][0], 0)  # redirect stdin to read end of previous pipe
            if i < num_cmd - 1:
                os.dup2(pipes[i][1], 1)
            for r, w in pipes:
                os.close(r)
            ...
            try: 
                ...
                print(f"Error occurred while executing {parts[0]}: {e}")

            os._exit(1)  # Exit the child process if execvp fails
...
def show_help(command):
    ...
    match command:
        ...
        case "cat":
            ...
            print("Concatenate and display FILE(s) content")
            print("       cat file.txt       - Display file content")
            print("       cat file1 file2    - Display multiple files content")

        case "touch":
            print("Usage: touch file1 file2 ...")
            print("Create an empty file or update the timestamp of an existing file")
            print("       touch file.txt     - Create or update file.txt")
            print("       touch file1 file2  - Create or update multiple files")
